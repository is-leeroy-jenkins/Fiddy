/*
 * ==========================================================================================
 *  Fiddy Documentation JavaScript
 *  File: docs/assets/js/fiddy.js
 *
 *  Purpose:
 *      Provides safe progressive enhancements for the Fiddy MkDocs Material site.
...
(function () {
	...
	const FiddyDocs = {
		config: {
			scrollTopId: "fiddy-scroll-top",
			pageToolsId: "fiddy-page-tools",
			progressId: "fiddy-reading-progress",
			tableFilterClass: "fiddy-table-filter",
			headingLinkClass: "fiddy-heading-link",
			codeLabelClass: "fiddy-code-label",
			codeToggleClass: "fiddy-code-toggle",
			initializedAttribute: "data-fiddy-enhanced",
			navScrollKey: "fiddy-docs-nav-scroll",
			...
			tableSelector: ".md-typeset table:not([data-fiddy-no-filter])",
		...
		handleDocumentClick: function (event) {
			...
			if (target.closest && target.closest("[data-fiddy-copy-heading]")) {
				event.preventDefault();
				this.copyHeadingLink(target.closest("[data-fiddy-copy-heading]"));
				return;
			}

			if (target.closest && target.closest("[data-fiddy-copy-page]")) {
				event.preventDefault();
				this.copyPageLink(target.closest("[data-fiddy-copy-page]"));
				return;
			}

			if (target.closest && target.closest("[data-fiddy-print-page]")) {
				event.preventDefault();
				window.print();
				return;
			}

			if (target.closest && target.closest("[data-fiddy-toggle-code]")) {
				event.preventDefault();
				this.toggleCodeBlock(target.closest("[data-fiddy-toggle-code]"));
				return;
			}

			if (target.closest && target.closest("[data-fiddy-api-expand]")) {
				event.preventDefault();
				this.setApiDetailsState(true);
				return;
			}

			if (target.closest && target.closest("[data-fiddy-api-collapse]")) {
				event.preventDefault();
				this.setApiDetailsState(false);
				return;
			}

			if (target.closest && target.closest("[data-fiddy-api-clear]")) {
				event.preventDefault();
				this.clearApiFilter();
			}
		},
		...
		handleKeyboardShortcuts: function (event) {
			...
			if (event.altKey && key === "f") {
				const apiSearch = document.getElementById("fiddy-api-search");
			...
			links.forEach(function (link) {
				try {
					...
					if (url.host && url.host !== currentHost) {
						...
						link.classList.add("fiddy-external-link");

						if (!link.querySelector(".fiddy-external-indicator")) {
							const indicator = document.createElement("span");
							indicator.className = "fiddy-external-indicator";
		...
		customizeSearch: function () {
			...
			searchInputs.forEach(function (input) {
				input.setAttribute("placeholder", "Search Fiddy docs...");
				input.setAttribute("aria-label", "Search Fiddy documentation");
			});
		},
		...
		addScrollTopButton: function () {
			...
			button.type = "button";
			button.className = "fiddy-scroll-top";
		...
		addPageTools: function () {
			...
			tools.className = "fiddy-page-tools";
			tools.innerHTML = [
				"<button type=\"button\" data-fiddy-copy-page title=\"Copy page link\" aria-label=\"Copy page link\">Copy link</button>",
				"<button type=\"button\" data-fiddy-print-page title=\"Print page\" aria-label=\"Print page\">Print</button>"
			].join("");
		...
		copyCurrentPageToClipboard: function () {
			const button = document.querySelector("[data-fiddy-copy-page]");
			this.copyTextToClipboard(window.location.href, button, "Copied", "Copy link");
		},
		...
		addHeadingLinks: function () {
			...
			headings.forEach(function (heading) {
				if (heading.querySelector("." + FiddyDocs.config.headingLinkClass)) {
					return;
				}

				const button = document.createElement("button");
				button.type = "button";
				button.className = FiddyDocs.config.headingLinkClass;
				button.setAttribute("data-fiddy-copy-heading", heading.id);
		...
		copyHeadingLink: function (button) {
			const id = button.getAttribute("data-fiddy-copy-heading");
		...
		copyTextToClipboard: function (text, button, successText, defaultText) {
			...
			if (navigator.clipboard && typeof navigator.clipboard.writeText === "function") {
				navigator.clipboard.writeText(text).then(updateButton).catch(function () {
					FiddyDocs.fallbackCopyText(text);
					updateButton();
				});
		...
		addTableFilters: function () {
			...
			tables.forEach(function (table, index) {
				if (table.getAttribute("data-fiddy-filtered") === "true") {
					return;
				}
				...
				if (rows.length < FiddyDocs.config.largeTableMinimumRows) {
					return;
				}

				table.setAttribute("data-fiddy-filtered", "true");

				const wrapper = document.createElement("div");
				wrapper.className = "fiddy-table-tools";

				const input = document.createElement("input");
				input.type = "search";
				input.className = FiddyDocs.config.tableFilterClass;
				input.placeholder = "Filter table...";
				input.setAttribute("aria-label", "Filter table " + (index + 1));

				const count = document.createElement("span");
				count.className = "fiddy-table-count";
				...
				input.addEventListener("input", function () {
					FiddyDocs.filterTable(table, input.value, count);
				});
			});
		},
		...
		addCodeLabels: function () {
			...
			codeBlocks.forEach(function (code) {
				...
				if (!pre || pre.getAttribute("data-fiddy-labeled") === "true") {
					return;
				}

				const language = FiddyDocs.detectCodeLanguage(code);

				if (!language) {
					return;
				}

				pre.setAttribute("data-fiddy-labeled", "true");

				const label = document.createElement("div");
				label.className = FiddyDocs.config.codeLabelClass;
		...
		addCodeToggles: function () {
			...
			codeBlocks.forEach(function (code) {
				...
				if (!pre || pre.getAttribute("data-fiddy-toggle-ready") === "true") {
					return;
				}

				pre.setAttribute("data-fiddy-toggle-ready", "true");

				if (pre.scrollHeight <= FiddyDocs.config.maxCollapsedCodeHeight + 80) {
					return;
				}

				pre.classList.add("fiddy-code-collapsed");
				pre.style.maxHeight = FiddyDocs.config.maxCollapsedCodeHeight + "px";

				const button = document.createElement("button");
				button.type = "button";
				button.className = FiddyDocs.config.codeToggleClass;
				button.setAttribute("data-fiddy-toggle-code", "collapsed");
		...
		toggleCodeBlock: function (button) {
			...
			const state = button.getAttribute("data-fiddy-toggle-code");

			if (state === "collapsed") {
				pre.classList.remove("fiddy-code-collapsed");
				pre.style.maxHeight = "";
				button.setAttribute("data-fiddy-toggle-code", "expanded");
		...
		addPagePathMetadata: function () {
			...
			if (!content || content.querySelector(".fiddy-page-path")) {
				return;
			}
			...
			meta.className = "fiddy-page-path";
		...
		enhanceKeyboardFocus: function () {
			document.body.addEventListener("keydown", function (event) {
				if (event.key === "Tab") {
					document.body.classList.add("fiddy-keyboard-mode");
				}
			});

			document.body.addEventListener("mousedown", function () {
				document.body.classList.remove("fiddy-keyboard-mode");
			});
		},
		...
		enhanceApiReference: function () {
			...
			apiContainers.forEach(function (container) {
				if (container.getAttribute("data-fiddy-api-enhanced") === "true") {
					return;
				}

				container.setAttribute("data-fiddy-api-enhanced", "true");

				const heading = container.querySelector("h2, h3, h4, h5");

				if (!heading || heading.querySelector(".fiddy-api-badge")) {
					return;
				}

				const badge = document.createElement("span");
				badge.className = "fiddy-api-badge";
		...
		addApiTools: function () {
			...
			if (content.querySelector(".fiddy-api-tools")) {
				return;
			}
			...
			panel.className = "fiddy-api-tools";
			panel.setAttribute("aria-label", "API tools");

			panel.innerHTML = [
				"<h2 class=\"fiddy-api-tools-title\">API Tools</h2>",
				"<label class=\"fiddy-api-search-label\" for=\"fiddy-api-search\">Filter classes, methods, properties, or text</label>",
				"<input id=\"fiddy-api-search\" class=\"fiddy-api-search\" type=\"search\" placeholder=\"Filter classes, methods, properties, or text...\" autocomplete=\"off\">",
				"<div class=\"fiddy-api-tool-buttons\">",
				"<button type=\"button\" class=\"fiddy-api-tool-button\" data-fiddy-api-expand>Expand all</button>",
				"<button type=\"button\" class=\"fiddy-api-tool-button\" data-fiddy-api-collapse>Collapse all</button>",
				"<button type=\"button\" class=\"fiddy-api-tool-button\" data-fiddy-api-clear>Clear filter</button>",
				"</div>",
				"<p class=\"fiddy-api-filter-status\" aria-live=\"polite\"></p>"
			].join("");

			firstHeading.insertAdjacentElement("afterend", panel);

			const input = panel.querySelector("#fiddy-api-search");
			const status = panel.querySelector(".fiddy-api-filter-status");

			if (input) {
				input.addEventListener("input", function () {
					FiddyDocs.filterApiObjects(input.value, status);
				});
			}
		},
		...
		filterApiObjects: function (query, statusElement) {
			...
			objects.forEach(function (object) {
				...
				if (!normalizedQuery || text.indexOf(normalizedQuery) !== -1) {
					object.classList.remove("fiddy-api-hidden");
					visibleCount += 1;
				} else {
					object.classList.add("fiddy-api-hidden");
				}
			});
		...
		clearApiFilter: function () {
			const input = document.getElementById("fiddy-api-search");
			const status = document.querySelector(".fiddy-api-filter-status");
		...
		enhanceTocProgress: function () {
			...
			if (!toc || toc.getAttribute("data-fiddy-toc-enhanced") === "true") {
				return;
			}

			toc.setAttribute("data-fiddy-toc-enhanced", "true");

			const marker = document.createElement("div");
			marker.className = "fiddy-toc-marker";
		...
		updateTocProgress: function () {
			...
			tocLinks.forEach(function (link) {
				...
				if (href === activeId) {
					link.classList.add("fiddy-toc-active");
				} else {
					link.classList.remove("fiddy-toc-active");
				}
			});
		},
		...
		addMermaidGuard: function () {
			...
			blocks.forEach(function (block) {
				block.setAttribute("data-fiddy-mermaid-detected", "true");
			});
		}
	};
