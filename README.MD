Regenerated from scratch below. I removed the explicit “Requirements Coverage” framing and wrote it
as a finished project README with a capability-oriented presentation, renderer-safe architecture
diagram, clean tables, and practical usage examples. Source materials were treated as background
context only.
...
submitted label artwork against application data. It combines AI-assisted local OCR, deterministic verification
rules, fuzzy matching, image-quality diagnostics, batch processing, and downloadable review outputs
...
[Azure](#-azure-deployment-posture) •
...
Fiddy is organized as a local-first verification pipeline. The interface, ingestion logic, OCR
processing, verification engine, reporting layer, and diagnostics are separated so the system
remains understandable, testable, and maintainable.

```text
┌──────────────────────────────────────────────────────────────────────────────┐
│                              FIDDY STREAMLIT UI                              │
│               app.py • Simple Mode • Advanced Mode • Downloads               │
└──────────────────────────────────────┬───────────────────────────────────────┘
                                       │
                  ┌────────────────────┴────────────────────┐
                  │                                         │
┌─────────────────▼─────────────────┐       ┌───────────────▼─────────────────┐
│        APPLICATION DATA            │       │          LABEL ARTWORK           │
│  CSV Manifest or Manual CAV Entry  │       │   Images • PDFs • ZIP Archives   │
└─────────────────┬─────────────────┘       └───────────────┬─────────────────┘
                  │                                         │
┌─────────────────▼─────────────────┐       ┌───────────────▼─────────────────┐
│        MANIFEST PROCESSING         │       │       TEMPORARY WORKSPACE        │
│ batch_manifest.py                  │       │ Session-only file handling        │
│ Parse • validate • match records   │       │ Uploads staged for OCR/review     │
└─────────────────┬─────────────────┘       └───────────────┬─────────────────┘
                  │                                         │
                  └────────────────────┬────────────────────┘
                                       │
┌──────────────────────────────────────▼───────────────────────────────────────┐
│                         BATCH AND LABEL ORCHESTRATION                        │
│                 batch_processor.py • label_verifier.py                       │
│           Match records • isolate files • execute per-label review            │
└──────────────────────────────────────┬───────────────────────────────────────┘
                                       │
┌──────────────────────────────────────▼───────────────────────────────────────┐
│                         OCR AND IMAGE PROCESSING PIPELINE                    │
│ image_processor.py • visual_quality.py • ocr_engine.py                       │
│        Load artwork • assess quality • preprocess • extract label text        │
└──────────────────────────────────────┬───────────────────────────────────────┘
                                       │
┌──────────────────────────────────────▼───────────────────────────────────────┐
│                             VERIFICATION ENGINE                              │
│ normalizer.py • label_rules.py • warning_validator.py • models.py            │
│      Normalize values • compare fields • assign status • explain findings     │
└──────────────────────────────────────┬───────────────────────────────────────┘
                                       │
┌──────────────────────────────────────▼───────────────────────────────────────┐
│                           REPORTING AND DIAGNOSTICS                          │
│ report_writer.py • performance_monitor.py • booger.py                        │
│      Summary • details • comparison • timing • optional exception logging      │
└──────────────────────────────────────┬───────────────────────────────────────┘
                                       │
┌──────────────────────────────────────▼───────────────────────────────────────┐
│                              REVIEWER OUTPUTS                                │
│             Dashboard • Side-by-side comparison • CSV • JSON • Markdown       │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
| Component                | Role                                                                                              |
|  | - |
| `app.py`                 | Streamlit interface, workflow routing, uploads, reviewer controls, result display, and downloads. |
| `config.py`              | Application settings, paths, upload limits, OCR settings, thresholds, assets, and logging paths.  |
| `batch_manifest.py`      | CSV manifest parsing, validation, filename matching, and conversion to application records.       |
| `batch_processor.py`     | Batch orchestration, per-file isolation, skipped-file tracking, and batch result assembly.        |
| `image_processor.py`     | Image loading, EXIF orientation correction, mode conversion, and OCR preparation.                 |
| `visual_quality.py`      | Blur, contrast, glare, darkness, skew, size, and readability diagnostics.                         |
| `ocr_engine.py`          | Local OCR extraction from supported image files and PDF pages.                                    |
| `normalizer.py`          | Text, punctuation, whitespace, ABV, proof, and unit normalization.                                |
| `label_rules.py`         | Field-level verification rules, fuzzy matching, numeric comparison, and result creation.          |
| `warning_validator.py`   | Government-warning presence, exact text, prefix, and visual-review handling.                      |
| `label_verifier.py`      | Single-label OCR coordination, rule execution, and verification report creation.                  |
| `performance_monitor.py` | Per-label timing and batch-level performance summaries.                                           |
| `report_writer.py`       | Summary, detail, comparison, CSV, JSON, and Markdown report generation.                           |
| `booger.py`              | Optional local SQLite exception logging for diagnostics.                                          |
...
Fiddy is a standalone prototype and is not an official records-management system.
...
Fiddy is a working prototype, not a production compliance platform.
...
Fiddy uses a compact Python dependency stack focused on Streamlit UI, local OCR, image processing,
...
This prototype is distributed under the MIT-style license included in the source files.



## ✅ Project Status

Fiddy is a completed prototype for evaluating AI-assisted alcohol label verification. It
demonstrates a practical reviewer workflow, local OCR, deterministic comparison logic, fuzzy
matching, batch review, human-review safeguards, and downloadable results.

Before production deployment, Fiddy should undergo security review, accessibility review,
performance validation, records-management review, and Azure deployment hardening.
